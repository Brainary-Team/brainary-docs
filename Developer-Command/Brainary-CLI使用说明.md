...
| `brainary app-server` | 实验性接口：启动 app-server，并提供 daemon、协议 Schema 和绑定生成工具 |
| `brainary remote-control` | 实验性接口：启停启用了远程控制的 app-server daemon，或生成短期配对码 |
| `brainary exec-server` | 实验性接口：启动独立 exec-server，可通过 stdio 或 WebSocket 接入 |
...
| 入口 | 当前边界 |
| --- | --- |
| `brainary apply` / `brainary cloud` | 保留的云端兼容能力，依赖已有有效凭据、服务接入和任务权限；不能视为默认可用的 Brainary 云服务 |
...
| `brainary app` | 不作为 Brainary 公共能力；它指向外部桌面产品，正式分发前应确保构建中不再展示该入口 |
...
```bash
brainary apply <TASK_ID>
```

该命令是保留的云端兼容入口，要求本机已经具备有效凭据、对应服务接入以及可访问的任务 ID。当前 Brainary 普通账号登录流程已暂停，因此不能假定新用户可以直接使用。满足条件时，它会从远端任务获取最新 diff，再通过 `git apply` 应用到当前 Git 工作区；它不能用于直接应用任意本地会话的修改。

执行前应确认当前工作目录和工作区状态，执行后使用 `git diff` 检查实际修改。
...
| `marketplace <COMMAND>` | `add`、`list`、`update`、`remove` | 添加、列出、升级或移除插件市场 |
...
# 启动、停止远程控制 daemon，或生成短期配对码
brainary remote-control start
brainary remote-control pair
brainary remote-control stop

# 启动独立 exec-server
brainary exec-server --listen stdio://
```

`app-server`、`remote-control` 和 `exec-server` 是实验性接口，适合客户端开发、自动化或协议调试。部署到非本机地址前必须另外设计传输安全、鉴权和生命周期管理，不能按普通用户命令直接对公网开放。
